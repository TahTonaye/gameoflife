"""Interface PySide6 : une grille cliquable, Play/Pause sur un QTimer.

Toute la logique du jeu vit dans game_of_life.engine — cette fenêtre
ne fait qu'afficher l'état (0/1) et déclencher les générations.

Usage :
    python main.py
    python -m game_of_life.gui
"""
...
class GameOfLife(QWidget):
    def __init__(self, size=20):
        ...
        # État logique du monde : 1 = vivante, 0 = morte.
    ...
    def reset_world(self):
        """Réinitialise la grille avec un glider chargé depuis patterns/."""
        self.timer.stop()
        self.running = False
        self.play_btn.setText("Play")
        self.generation = 0

        # Motif de départ : glider.csv, posé au centre de la grille.
        glider = load_pattern("glider")
        top = (self.size - glider.shape[0]) // 2
        left = (self.size - glider.shape[1]) // 2

        empty = np.zeros((self.size, self.size), dtype=int)
        self.world = place(empty, glider, top, left)
    ...
    def random_world(self):
        ...
        self.world = (np.random.rand(self.size, self.size) < 0.28).astype(int)
    ...
    def on_click(self, r, c):
        ...
        self.world[r, c] = 0 if self.world[r, c] else 1
...
def main():
    """Lance l'application Qt (une seule par programme)."""
    app = QApplication(sys.argv)

    window = GameOfLife(size=20)
    window.show()
